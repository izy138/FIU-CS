...
echo "=========================================================================="
echo "                  HOMEWORK-6: Batch Autograder                            "
echo "=========================================================================="
echo "Date: $(date)"
echo

# Check if autograder script exists
if [ ! -f "autograder_HW6.sh" ]; then
    echo "ERROR: autograder_HW6.sh not found in current directory!"
...
echo "COP4610 Assignment - Grading Summary" > "$SUMMARY_FILE"
...
REQUIRED_C_FILES=("submission_HW6.c")
...
for zip_file in "${zip_files[@]}"; do
    ...
    # Always ignore student Makefiles
    log_message "    📌 Using instructor-provided framework Makefile (any student Makefile ignored)"
    ...
    found_readme=$(find "$TEMP_DIR" -name "README.pdf" -type f ! -path "*/__MACOSX/*" | head -1)
	if [ -z "$found_readme" ]; then
        #found_readme=$(find "$TEMP_DIR" -name "README.txt" -type f | head -1)
		found_readme=$(find "$TEMP_DIR" -name "README.txt" -type f ! -path "*/__MACOSX/*" | head -1)
    fi
    if [ -z "$found_readme" ]; then
        #found_readme=$(find "$TEMP_DIR" -name "README.md" -type f | head -1)
		found_readme=$(find "$TEMP_DIR" -name "README.md" -type f ! -path "*/__MACOSX/*" | head -1)
    fi
    ...
    if [ -n "$found_readme" ]; then
        ...
        log_message "    ❌ Missing: README file (expected README.pdf or README.txt or README.md)"
    fi

    SHOULD_GRADE=true

    if [ ${#FOUND_FILES[@]} -eq 0 ]; then
        log_message "  ⚠️ ERROR: No submission of \"submission_HW6.c\" found"
    ...
    if [ "$SHOULD_GRADE" = true ]; then
        ...
        AUTOGRADER_OUTPUT=$(timeout 30 ./autograder_HW6.sh 2>&1)
        ...
        if [ $AUTOGRADER_EXIT_CODE -eq 0 ]; then
            ...
            if [ -n "$ORIGINAL_SCORE" ] && [ -n "$ORIGINAL_TOTAL" ]; then
                ...
                PERCENTAGE=$(( FINAL_SCORE * 100 / 90 ))
                echo "$STUDENT_NAME: $FINAL_SCORE/90 ($PERCENTAGE%)" >> "$SUMMARY_FILE"
                log_message "  Score: $FINAL_SCORE/90 ($PERCENTAGE%)"
                [ "$FINAL_SCORE" -eq 90 ] && PERFECT_SCORES=$((PERFECT_SCORES + 1))
