/**
 * ===============================================================================
 * RR SCHEDULER - DRIVER PROGRAM
 * ===============================================================================
 * @file driver.c
 * @brief Driver program for RR scheduling homework
...
bool read_processes_from_stdin(SchedulerContext *ctx)
{
    ...
    while (line_number < 6 && fgets(line, sizeof(line), stdin))
    {
        line_number++;
    }
...
void calculate_average_times(const SchedulerContext *ctx)
{
    ...
    printf("AWT:%.2f\nATT:%.2f\n", (double)total_waiting_time / ctx->num_processes,
           (double)total_turnaround_time / ctx->num_processes);
}
...
int main(void)
{
    ...
    //💡 Run Round Robin (RR) scheduling with time quantum = 4

    round_robin_scheduling(&ctx, 4);
