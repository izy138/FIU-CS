...
echo "=========================================================================="
echo "                  HOMEWORK-8: Batch Autograder                            "
echo "=========================================================================="
echo "Date: $(date)"
echo

# Check if autograder script exists
if [ ! -f "autograder_HW8.sh" ]; then
    echo "ERROR: autograder_HW8.sh not found in current directory!"
...
REQUIRED_FILES=("driver.c" "page_replacement.h")
...
echo "COP4610 Homework 8 - Grading Summary" > "$SUMMARY_FILE"
...
REQUIRED_C_FILES=("submission_HW8.c")
...
for zip_file in "${zip_files[@]}"; do
    ...
    log_message "    📌 Using instructor-provided framework files (any student files ignored)"
    ...
    if [ ${#FOUND_FILES[@]} -eq 0 ]; then
        log_message "  ⚠️ ERROR: No submission of \"submission_HW8.c\" found"
    ...
    if [ "$SHOULD_GRADE" = true ]; then
        ...
        rm -f page_replacement *.o 2>/dev/null
    ...
    if [ "$SHOULD_GRADE" = true ]; then
        ...
        AUTOGRADER_OUTPUT=$(timeout 60 ./autograder_HW8.sh 2>&1)
        ...
        if [ $AUTOGRADER_EXIT_CODE -eq 0 ]; then
            ...
            if [ -n "$ORIGINAL_SCORE" ] && [ -n "$ORIGINAL_TOTAL" ]; then
                ...
                echo "$STUDENT_NAME: $FINAL_SCORE/$ORIGINAL_TOTAL ($PERCENTAGE%)" >> "$SUMMARY_FILE"
				log_message "  Score: $FINAL_SCORE/$ORIGINAL_TOTAL ($PERCENTAGE%)"
