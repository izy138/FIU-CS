...
# =============================================================================
ASSIGNMENT_NAME="HOMEWORK-3: Process Controller"
TOTAL_POINTS=100

# Test case weights (must sum to exactly 100)
declare -A TEST_WEIGHTS=(
    ["1"]=30    # Test 1 worth 30 points (Valid commands)
    ["2"]=30    # Test 2 worth 30 points (Invalid commands)
    ["3"]=40    # Test 3 worth 40 points (Mixed commands)
)

# # Test case weights (must sum to exactly 100)
# declare -A TEST_WEIGHTS=(
#     ["1"]=100    # Test 1 worth 100 points (Valid commands)   
# )
...
# Function to normalize PID output
normalize_pids() {
    local file="$1"
    # Replace <any_number> with <PID>
    sed 's/<[0-9]\+>/<PID>/g' "$file"
}

# NEW: Function to verify timeout exit codes by actually testing
verify_timeout_exit_codes() {
    local test_num=$1
    local student_output=$2
    
    # Check if this test case has long-running commands (sleep > 3 seconds)
    local input_file="Testing/Testcases/input${test_num}.txt"
    local has_long_sleep=false
    
    while IFS= read -r line; do
        if [[ $line =~ sleep[[:space:]]+([0-9]+) ]]; then
            local sleep_time=${BASH_REMATCH[1]}
            if [[ $sleep_time -gt 3 ]]; then
                has_long_sleep=true
                break
            fi
        fi
    ...
    # If no long sleep commands, no timeout verification needed
    if [[ $has_long_sleep == false ]]; then
        return 0  # Success (no timeout expected)
    fi
    
    # Test timeout behavior with a simple long-running command
    echo "sleep 10" > "Results/timeout_test_${test_num}.txt"
    local timeout_output="Results/timeout_test_output_${test_num}.txt"
    
    # Run student's program with timeout test
    timeout 15 ./"$EXECUTABLE_NAME" "Results/timeout_test_${test_num}.txt" > "$timeout_output" 2>/dev/null
    local exit_code=$?
    
    # Check if the output shows proper SIGALRM termination
    if grep -q "terminated by SIGALRM" "$timeout_output"; then
        return 0  # Success - proper timeout handling
    else
        echo -e "${RED}❌ TIMEOUT VERIFICATION FAILED: Long-running commands must be terminated with SIGALRM, not normal exit${NC}"
        return 1  # Failure - improper timeout handling
    fi
}
...
calculate_proportional_score() {
    ...
    # NEW: Check for proper timeout exit codes FIRST
    if ! verify_timeout_exit_codes "$test_num" "$student_output"; then
        echo "0"  # Automatic failure for incorrect timeout handling
        return
    fi
    
    # Normalize both files for PID comparison
    local normalized_expected="Results/normalized_expected_${test_num}.txt"
    local normalized_student="Results/normalized_student_${test_num}.txt"
    
    normalize_pids "$expected_file" > "$normalized_expected"
    normalize_pids "$student_output" > "$normalized_student"
    
    # Check if normalized outputs are identical (full credit)
    if diff -w -B -Z "$normalized_expected" "$normalized_student" > /dev/null 2>&1; then
        echo "1.0"
        return
    fi
    ...
    if [[ -f "$normalized_expected" ]]; then
        expected_content=$(cat "$normalized_expected")
    fi
    
    if [[ -f "$normalized_student" ]]; then
        student_content=$(cat "$normalized_student")
    fi
...
show_detailed_diff() {
    ...
    # Create detailed diff using normalized files
    local normalized_expected="Results/normalized_expected_${test_num}.txt"
    local normalized_student="Results/normalized_student_${test_num}.txt"
    local diff_file="Results/diff_${test_num}.txt"
    
    diff -u --label "Expected Output" --label "Your Output" "$normalized_expected" "$normalized_student" > "$diff_file" 2>/dev/null
    ...
    # Show statistics
    local expected_lines=$(wc -l < "$normalized_expected" 2>/dev/null || echo "0")
    local student_lines=$(wc -l < "$normalized_student" 2>/dev/null || echo "0")
...
check_files() {
    ...
    if [[ ! -f "$STUDENT_FILE" ]]; then
        ...
        echo "Make sure your implementation $STUDENT_FILE is in the current directory."
        exit 1
    fi
