...
# =============================================================================
ASSIGNMENT_NAME="Creating Static Library"
TOTAL_POINTS=100

# Test case weights (must sum to exactly 100)
declare -A TEST_WEIGHTS=(
    ["1"]=100    # Test 1 worth 100 points    
)
...
calculate_proportional_score() {
    ...
    if diff -w -B -Z "$expected_file" "$student_output" > /dev/null 2>&1; then
        echo "1.0"
        return
    fi
...
check_files() {
    ...
    if [[ ! -f "$STUDENT_FILE" ]]; then
        ...
        echo "Make sure your static library $STUDENT_FILE is in the current directory."
        exit 1
    fi
