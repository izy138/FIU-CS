...
if [ ! -f "autograder_HW7.sh" ]; then
    echo "ERROR: autograder_HW7.sh not found in current directory!"
...
REQUIRED_FILES=("driver.c" "synchronization.h")
...
REQUIRED_C_FILES=("submission_HW7.c")
...
for zip_file in "${zip_files[@]}"; do
    ...
    if [ ${#FOUND_FILES[@]} -eq 0 ]; then
        log_message "  ⚠️ ERROR: No submission of \"submission_HW7.c\" found"
    ...
    if [ "$SHOULD_GRADE" = true ]; then
        ...
        AUTOGRADER_OUTPUT=$(timeout 60 ./autograder_HW7.sh 2>&1)
