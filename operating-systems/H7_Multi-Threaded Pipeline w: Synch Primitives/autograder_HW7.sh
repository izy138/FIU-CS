...
# =============================================================================
ASSIGNMENT_NAME="HOMEWORK-7: Multi-Threaded Number Processing Pipeline"
TOTAL_POINTS=90
...
run_synchronization_tests() {
    ...
    echo "🔬 Test 2.1: Deadlock detection (10 runs of test case 1) - ${DEADLOCK_POINTS} points:"
    ...
    if [[ "$DEADLOCK_DETECTED" = false ]]; then
        print_success "No deadlock detected in 10 runs"
        DEADLOCK_SCORE=1.0
    else
    ...
    echo "🔬 Test 2.2: Race condition detection (20 runs for consistency) - ${RACE_CONDITION_POINTS} points:"
    ...
    if [[ $baseline_exit -ne 0 && $baseline_exit -ne 124 ]]; then
        ...
        if [[ "$RACE_DETECTED" = false ]]; then
            print_success "Consistent output across 20 runs (no race conditions)"
...
generate_results() {
    ...
    for ((i=0; i<${#test_numbers[@]}; i++)); do
        ...
        printf "  Test Case %s (input%s.txt): %.1f/%.0f points (%.1f%%)\n" "$test_num" "$test_num" "$points" "$test_weight" "$percentage"
    done
    ...
    printf "  Test 2.1 (Deadlock detection): %.1f/%.0f points (%.1f%%)\n" "$deadlock_points" "$DEADLOCK_POINTS" "$deadlock_percentage"
    ...
    printf "  Test 2.2 (Race condition detection): %.1f/%.0f points (%.1f%%)\n" "$race_points" "$RACE_CONDITION_POINTS" "$race_percentage"
