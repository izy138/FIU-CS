...
# =============================================================================
ASSIGNMENT_NAME="HOMEWORK-4: TCP Command Server"
TOTAL_POINTS=100
...
# Function to normalize PID and port output
normalize_output() {
    local file="$1"
    # Replace <any_number> with <PID>
    sed -e 's/<[0-9]\+>/<PID>/g' \
        -e 's/127\.0\.0\.1:[0-9]\+/127.0.0.1:XXXXX/g' \
        -e 's/Child [0-9]\+:/Child <PID>:/g' \
        -e 's/Child [0-9]\+ /Child <PID> /g' \
...
calculate_proportional_score() {
    ...
    # Normalize both files for PID and port comparison
    ...
    normalize_output "$expected_file" > "$normalized_expected"
    normalize_output "$student_output" > "$normalized_student"
...
validate_test_files() {
    ...
    for test_num in "${!TEST_WEIGHTS[@]}"; do
        ...
        if [[ ! -f "$input_file" ]]; then
            print_error "Test case input file $input_file not found"
            exit 1
        fi
        
        if [[ ! -f "$expected_file" ]]; then
            print_error "Expected output file $expected_file not found"
            exit 1
        fi
    done
...
compile_code() {
    ...
    if [[ ! -x "$SERVER_EXECUTABLE" || ! -x "$CLIENT_EXECUTABLE" ]]; then
        print_error "Executables $SERVER_EXECUTABLE or $CLIENT_EXECUTABLE not created"
        exit 1
    fi
}
...
run_tests() {
    echo
    echo "🧪 Running TCP server test with proportional scoring..."
    ...
    for test_num in "${test_numbers[@]}"; do
        ...
        local server_output="Results/server_output_${test_num}.txt"
        ./"$SERVER_EXECUTABLE" > "$server_output" 2>&1 &
        local server_pid=$!
        
        # Give server time to start
        sleep 1
        
        # Run client and capture combined output
        local client_output="Results/client_output_${test_num}.txt"
        local combined_output="Results/combined_output_${test_num}.txt"
        
        timeout 10 ./"$CLIENT_EXECUTABLE" "$input_file" > "$client_output" 2>&1
        ...
        {
            cat "$server_output" 2>/dev/null
            cat "$client_output" 2>/dev/null
        } > "$combined_output" || true"$combined_output"
        ...
        if [[ $client_exit_code -eq 124 ]]; then
            ...
            crashed=1
        elif [[ $client_exit_code -gt 128 ]]; then
            print_error "Test $test_num CRASHED"
            crashed=1
        fi
        
        # Calculate proportional score for this test
        local score=$(calculate_proportional_score "$test_num" "$expected_file" "$combined_output" "$crashed")
        ...
        if [[ $crashed -eq 1 ]]; then
            ...
            show_detailed_diff "$test_num" "$expected_file" "$combined_output" "$score" "$test_weight"
        fi
